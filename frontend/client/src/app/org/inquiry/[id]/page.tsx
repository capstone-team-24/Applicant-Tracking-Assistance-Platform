...
export default function OrgInquiryPage() {
  ...
  const addFiles = (incoming: FileList | null) => {
    ...
    const combined = [...newFiles, ...arr].slice(0, MAX_FILES);
    const oversized = combined.filter((f) => f.size > MAX_SIZE_MB * 1024 * 1024);
    if (oversized.length) { setFileError(`"${oversized[0].name}" exceeds ${MAX_SIZE_MB} MB.`); return; }
    setFileError(""); setNewFiles(combined);
  };
    ...
    return (
      <div className="min-h-screen w-full bg-cover bg-center bg-fixed flex items-center justify-center px-4 relative" style={{ backgroundImage: `url(/contact-us.jpg)` }}>
        ...
        <div className="relative z-10 max-w-md w-full bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl p-10 text-center shadow-2xl">
          ...
          <Link href="/" className="mt-8 inline-block px-6 py-3 bg-white text-slate-900 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 transition-all">Return Home</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen w-full bg-cover bg-center bg-fixed flex items-center justify-center px-4 relative" style={{ backgroundImage: `url(/contact-us.jpg)` }}>
        ...
        <div className="relative z-10 max-w-md w-full bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl p-10 text-center shadow-2xl">
          ...
          <p className="text-white/60 text-sm leading-relaxed mb-2">Your updated details have been received. Our team will review them shortly.</p>
          <p className="text-white/30 text-xs leading-relaxed">
            We&apos;ll reach out at <strong className="text-white/50">{submission?.email}</strong> with our decision.
          </p>
          <Link href="/" className="mt-8 inline-block px-6 py-3 bg-white text-slate-900 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 transition-all">Return Home</Link>
        </div>
      </div>
        ...
        <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="w-full px-4 py-3 bg-white/[0.03] border border-white/5 rounded-xl text-white/40 text-sm italic cursor-not-allowed">{submission?.name}</div>
            </div>

            <div>
              <label htmlFor="hrAdminName" className="block text-xs font-bold text-white/60 uppercase tracking-widest mb-1">HR Admin Name</label>
              <input id="hrAdminName" type="text" placeholder="Jane Smith"
                ...
                value={form.hrAdminName} onChange={(e) => setForm({ ...form, hrAdminName: e.target.value })} />
            </div>

            <div>
              <label htmlFor="companyDetails" className="block text-xs font-bold text-white/60 uppercase tracking-widest mb-1">Company Details</label>
              <textarea id="companyDetails" rows={3} placeholder="Industry, company size, hiring needs..."
                ...
                value={form.companyDetails} onChange={(e) => setForm({ ...form, companyDetails: e.target.value })} />
            </div>
            ...
            <div>
              ...
              <textarea id="message" rows={5} required placeholder="Provide your updated message / clarification here..."
                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:bg-white/10 transition-all text-sm resize-none"
              ...
              <div>
                ...
                <div className="space-y-2">
                  {existingDocs.map((doc) => (
                    <div key={doc.id} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white/30 text-[10px]">{formatSize(doc.fileSize ?? 0)}</p>
                      </div>
                      <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400/60 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">Uploaded</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              <label className="block text-xs font-bold text-white/60 uppercase tracking-widest mb-2">
                {existingDocs.length > 0 ? "Add More Documents" : "Verification Documents"}
              </label>
              <p className="text-white/30 text-xs mb-3">
                PDF, DOC, DOCX, PNG, JPG — max {MAX_SIZE_MB} MB each, up to {MAX_FILES} files total
              </p>
              ...
              <div
                ...
                className="border-2 border-dashed border-white/15 hover:border-blue-400/50 rounded-2xl p-5 text-center cursor-pointer transition-all group"
              >
                <svg className="w-7 h-7 text-white/20 group-hover:text-blue-400/60 mx-auto mb-2 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                </svg>
                <p className="text-white/40 text-xs group-hover:text-white/60 transition-colors">Click to browse or drag & drop</p>
                <input ref={fileInputRef} type="file" multiple accept={ACCEPTED} className="hidden"
                  onChange={(e) => addFiles(e.target.files)} />
              </div>

              {fileError && <p className="text-rose-400 text-xs mt-2">{fileError}</p>}
